...
- **改写模型连接（外部模型 V1）**：系统级连接列表 + 新建/编辑抽屉。支持三种来源——内置本地 Qwen（默认，随 rewriter 容器部署）、智谱 GLM（经官方 `zai-sdk` 调用；**端点档位**可选「通用开放平台」（按量计费，消耗 API 余额）或「Coding Plan 专用端点」（消耗订阅额度——官方条款仅限指定编码工具使用，本产品以自身身份如实调用，可能被服务端拒绝，订阅额度与 API 余额互不通用）；`thinking` 按模型能力设置——常规模型默认关闭、可开，始终思考模型如 `glm-5.3-flash` 强制开启且不可关；JSON mode 默认开启）、OpenAI 兼容 API（自定义 Base URL，仅允许 https 公网地址）；
