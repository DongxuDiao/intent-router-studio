...
- **改写模型连接（外部模型 V1）**：系统级连接列表 + 新建/编辑抽屉。支持三种来源——内置本地 Qwen（默认，随 rewriter 容器部署）、智谱 GLM（经官方 `zai-sdk` 调用，端点固定为官方通用开放平台地址；`thinking` 按模型能力设置——常规模型默认关闭、可开，始终思考模型如 `glm-5.3-flash` 强制开启且不可关；JSON mode 默认开启）、OpenAI 兼容 API（自定义 Base URL，仅允许 https 公网地址）；
